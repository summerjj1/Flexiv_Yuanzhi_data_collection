...
from REAL2SIM_GELLO_AIR_v6 import HLSServoController
...
class TrajectoryRecorder:
    ...
    def init_realsense(self):
        ...
        self.logger.info(f'self.serial_numbers{self.serial_numbers}')
        cameras =  {"hand":'313522302661', 
                   "left":'336522303555',
                   "right":'247122072727'}
        if not cameras:
...
def main(task, path, frequency, rgb_width=640, rgb_height=480, fps=30, gui=False, enable_realsense=True, debug_sensor=False, home_target=None):
    ...
    gellor_controller.init_hls_port()
    # gellor_controller.spring_damper_to_gello_home(iterations=100, control_interval=0.01)

    custom_target = np.array([-1.05, -41.52, 12.43, 74.35, -13.05, 29.67, 14.14]) * np.pi/ 180.0

    custom_target = (np.array(custom_target.tolist() + [np.pi]) + np.pi) / np.pi * 2048
    gellor_controller.spring_damper_to_custom_position(
        target_positions=custom_target,
        iterations=50,
        control_interval=0.01
    )
    gellor_controller.disable_torque_control()
    ...
    while True:
        ...
        if not record_keyinput_flag:
            # continue
            right_robot.joint_position_control_with_impedance(target_positions, 0.5, vel_ratio=1.6, acc_ratio=0.3)
        else:   
...
if __name__ == "__main__":
    ...
    default_path = f"/home/vla/code/data/push_bottles/{current_date}/"
