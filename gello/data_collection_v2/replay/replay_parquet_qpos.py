...
def main(args):
    ...
    try:
        ...
        import  pandas  as pd
        traj_path = "/home/ubuntu/Downloads/episode_000000.parquet"
        df = pd.read_parquet(traj_path)
        traj_pose = np.stack(df['observation.state'].values)        
        traj_gripper =  np.stack(df['action'].values)[:, -1]
        ...
        for pose, gripper_width in tqdm(zip(traj_pose, traj_gripper)):
            pose = pose.tolist()
            robot.SendJointPosition(pose, target_vel, MAX_VEL, MAX_ACC)

            if gripper_width > 0.5:
                gripper.Move(0.14, 0.1, 50)
            else:
                gripper.Move(0, 0.1, 50)
            time.sleep(1 / 10)  # Adjust based on trajectory timing
...
if __name__ == "__main__":
    ...
    parser.add_argument('--robot_serial', type=str, default="Rizon 4 -00015", 
                        help="Serial number of the robot (default: Rizon 4 -00015)")
    ...
    parser.add_argument('--h5_file', type=str, default="/home/ubuntu/zhq/flexiv_data_collect/gello/data_collection_v2/trajectory_20251224_173218.h5",
