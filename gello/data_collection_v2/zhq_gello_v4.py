...
record_keyinput_flag = 1 # 1 初始 2-开始才数据 
def on_press(key):
    global record_keyinput_flag, key_input
    try:
        if key.char is 'i':
            record_keyinput_flag = 2
        if key.char is 'b':
            record_keyinput_flag = 3            
        if record_keyinput_flag and Key.space:
...
class TrajectoryRecorder:
    ...
    def init_realsense(self):
        """Initialize two RealSense cameras using the Sensor interface."""
        ...
        if len(available_keys) < 3:
            self.logger.warn(f"Only {len(available_keys)} RealSense cameras detected, need at least 3")
            return []
        
        self.serial_numbers = available_keys[:3]  # 取前两个相机
        
        # 分配两个角色："rs_cam1" 和 "rs_cam2"
        cameras = {
            "rs_cam1": self.serial_numbers[0],
            "rs_cam2": self.serial_numbers[1],
            "rs_cam3": self.serial_numbers[2]
        }
        if not cameras:
        ...
        try:
            ...
            if len(test_data) >= num_cameras:
                # 检查两个相机的图像形状
                shapes = []
                for i in range(min(num_cameras, len(test_data))):
                    if test_data[i] is not None:
                        shapes.append(test_data[i].shape)
                
                if len(shapes) > 0:
                    # 使用第一个相机的形状作为参考
                    self.realsense_image_shape = shapes[0]
                    self.logger.info(f"RealSense image shape (reference): {self.realsense_image_shape}")
                    
                    # 记录每个相机的形状
                    for i, shape in enumerate(shapes):
                        cam_name = list(cameras.keys())[i]
                        self.logger.info(f"Camera {cam_name} shape: {shape}")
                else:
                    raise RuntimeError("No valid frames received during initialization")
            else:
                raise RuntimeError(f"Expected at least 3 cameras, got {len(test_data)} frames")
        except Exception as exc:
            self.logger.error(f"Failed to get RealSense image shapes: {exc}")
            self.realsense_image_shape = (image_height, image_width, 3)

        return list(cameras.keys())  # 返回 ["rs_cam1", "rs_cam2", "rs_cam3"]
    ...
    def _state_loop(self, fps=100):
        ...
        while self._state_running:
            try:
                ...
                timestamp = time.time()
                # print("read gripper_states: ", gripper_states.width)
    ...
    def add_state(self, robot_states, gripper_states, timestamp=None):
        ...
        try:
            ...
            # --- 修改结束 ---
            self.action_list.append([float(i) for i in robot_states.tcp_pose])  # Placeholder for action alignment
            ...
            # === 修改开始: 夹爪数据二值化处理 ===
            raw_width = float(gripper_states.width)
            # 逻辑：大于 0.05 记为 1 (Open)，否则记为 0 (Closed)
            processed_width = 1.0 if raw_width > 0.05 else 0.0
            
            self.gripper_width_list.append(processed_width)
            # === 修改结束 ===
    ...
    def save_trajectory(self, task, key_input):
        ...
        with h5py.File(self.output_file, 'w', libver='latest', rdcc_nbytes=1024*1024*100) as hf:
            ...
            # Save RealSense images sequentially (避免线程安全问题)
            for cam_name in self.realsense_roles:
                ...
                self.logger.info(f"Saved {cam_name}: {images.shape[0]} frames, shape {images.shape[1:]}")
    ...
    def _save_camera_videos(self):
        ...
        # Save each RealSense camera video
        for cam_name in self.realsense_roles:
            if cam_name in self.camera_images_list and len(self.camera_images_list[cam_name]) > 0:
                # 获取该相机的图像形状
                first_image = self.camera_images_list[cam_name][0]
                if isinstance(first_image, np.ndarray):
                    height, width = first_image.shape[:2]
                else:
                    height, width = 480, 640  # 默认值
                
                video_path = f"{base_path}_realsense_{cam_name}.mp4"
                
                self._save_single_camera_video(
                    cam_name, video_path, fps, width, height,
    ...
    def _save_single_camera_video(self, cam_name, video_path, fps, width, height, images, valid_mask, camera_type):
        ...
        # 使用MP4V而不是avc1，因为某些系统可能没有H.264编码器
        codecs_to_try = [('mp4v', 'MPEG-4'), ('XVID', 'Xvid'), ('MJPG', 'Motion-JPEG')]
        ...
        try:
            ...
            for frame_idx, (image, is_valid) in enumerate(zip(images, valid_mask)):
                ...
                if not isinstance(image, np.ndarray):
                    self.logger.warn(f"  {camera_type} {cam_name}: Frame {frame_idx} is not numpy array, skipping")
                    continue
                ...
                if len(image.shape) == 2:  # Grayscale
                    ...
                    self.logger.warn(f"  {camera_type} {cam_name}: Frame {frame_idx} has invalid shape {image.shape}, converting to BGR")
...
def main(task, path, frequency, rgb_width=640, rgb_height=480, fps=30, gui=False, enable_realsense=True, debug_sensor=False, home_target=None):
    ...
    if enable_realsense:
        # Initialize single RealSense CameraConfig
        realsense_config = CameraConfig(
            real_time_view=False,  # 可选：关闭实时显示以节省资源
            rgb_size=(rgb_width, rgb_height),
            depth_size=(rgb_width, rgb_height),
            fps=fps,
            save_freq=frequency
        )
        logger.info("Two RealSense cameras enabled (rs_cam1 and rs_cam2)")
    else:
        logger.warn("RealSense camera disabled; running without camera recordings")
    robot_config = RobotConfig(record_fps=400) # zhq modify 

    recorder = TrajectoryRecorder(output_file, realsense_config, robot_config=robot_config)


    gellor_controller = HLSServoController(dev_port="/dev/ttyUSB0", baud_rate=1000000)
    ...
    right_robot = flexiv_robot("Rizon 4-00015", "GripperDahuan")
            # initialize gripper
    gripper = flexivrdk.Gripper(right_robot.robot)
    ...
    recorder.start_state_thread(right_robot.robot, gripper, tac)
    ...
    pos8 = gellor_controller.read_current_positions_rad()
    ...
    right_robot.joint_position_control_with_impedance(target_positions, 0.1, vel_ratio=0.1, acc_ratio=0.1)
    time.sleep(2)



    while record_keyinput_flag == 1:
    ...
    while True:
        ...
        robot_states, current_tcp_pos, current_tcp_quat, gripper_states = get_cur_pose(right_robot.robot, gripper)
        qjoint = robot_states.q

        
        # if not record_keyinput_flag:
        if record_keyinput_flag == 2:
            # continue
            right_robot.joint_position_control_with_impedance(target_pos=target_positions, k_p_scale=0.5, vel_ratio=1.6, acc_ratio=0.3)
        elif record_keyinput_flag == 3:   
            print('Finishing recording ')
            right_robot.joint_position_control_with_impedance(target_pos=home_target, k_p_scale=0.5, vel_ratio=1.6, acc_ratio=0.3)
        ...
        if pos8[-1] > 0:
            ...
            raise ValueError("Unexpected gripper width value")
        gripper.Move(gripper_width, 0.1, 70)
...
def fast_map_gripper(x, scale=1.0/(0.6+0.44), offset=0.44/(0.6+0.44)):
    ...
    """
    clamped = max(0, min(0.145, x))
    clamped = (clamped + 0.44) * scale
...
if __name__ == "__main__":
    ...
    # default_path = f"/media/ubuntu/ZhqSSD/YZdata/insert_hole_threecam_24/{current_date}/"
    default_path = f"/media/ubuntu/ZhqSSD/YZdata/insert_hole_threecam/{current_date}/"
    # default_path = f"./"
    ...
    #geer_demo
    HOME_TARGET = [-0.05, -0.01, 0.11, 1.67, -0.05, 0.12, 0.06]
