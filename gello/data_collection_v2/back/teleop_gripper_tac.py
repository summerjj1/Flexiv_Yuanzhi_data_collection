...
def main(task, path, frequency, rgb_width=640, rgb_height=480, fps=30, gui=False, enable_realsense=True, debug_sensor=False, home_target=None):
    ...
    try:
        ...
        while True:
            ...
            # right_robot.robot.SendJointPosition(target_init_positions, target_vel, target_acc, MAX_VEL, MAX_ACC)
            right_robot.set_joint_positions(target_positions)
            # print(pos8)
            gripper_width = fast_map_gripper(pos8[-1])
            # if cnt % 10 == 0:
            print("++++++++++++++++++++++++++++gripper_width: ", gripper_width)
            cnt += 1
            # right_robot.gripper.Move(gripper_width * gripper_max_width, 0.1, 50)
            print(gripper_width)
