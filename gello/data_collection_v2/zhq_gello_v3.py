...
class TrajectoryRecorder:
    ...
    def init_realsense(self):
        ...
        if len(available_keys) < 2:
            self.logger.warn(f"Only {len(available_keys)} RealSense cameras detected, need at least 2")
            return []
        
        self.serial_numbers = available_keys[:2]  # 取前两个相机
        ...
        try:
            ...
            if len(test_data) >= 2:
                ...
                raise RuntimeError(f"Expected at least 2 cameras, got {len(test_data)} frames")
        except Exception as exc:
            self.logger.error(f"Failed to get RealSense image shapes: {exc}")
            self.realsense_image_shape = (image_height, image_width, 3)

        return list(cameras.keys())  # 返回 ["rs_cam1", "rs_cam2"]
...
if __name__ == "__main__":
    ...
    default_path = f"/media/ubuntu/ZhqSSD/YZdata/insert_hole_twocam_24/{current_date}/"
    # default_path = f"./"
