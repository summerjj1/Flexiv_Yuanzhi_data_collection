...
class TrajectoryRecorder:
    ...
    def init_realsense(self):
        """Initialize single RealSense camera using the Sensor interface."""
        ...
        # 只使用第一个检测到的相机
        self.serial_numbers = list(available.keys())[:1]  # 改为只取一个
        if len(self.serial_numbers) == 0:
            self.logger.warn("No RealSense serial numbers resolved")
            return []
        
        # 只分配一个角色"hand"
        cameras = {"rs_cam2": self.serial_numbers[0]}
        ...
        self.logger.info("Warming up RealSense camera...")
        for _ in range(10):
            try:
        ...
        try:
            ...
            if len(test_data) > 0:
                self.realsense_image_shape = test_data.shape  # (H, W, C)
        ...
        return list(cameras.keys())  # 返回 ["hand"]
        ...
        while self._camera_running:
            ...
            for i in range(self.num_realsense_cameras):
                if realsense_valid[i] and realsense_data is not None:
                    try:
                        image = np.array(realsense_data, copy=True)
...
def main(task, path, frequency, rgb_width=640, rgb_height=480, fps=30, gui=False, enable_realsense=True, debug_sensor=False, home_target=None):
    ...
    if enable_realsense:
        ...
        logger.info("Single RealSense camera enabled (rs_cam2)")
    else:
...
if __name__ == "__main__":
    ...
    default_path = f"/media/ubuntu/ZhqSSD/YZdata/insert_hole_v1/{current_date}/"
    # default_path = f"./"
