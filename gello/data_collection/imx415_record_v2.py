...
class IMX415CameraConfig:
    ...
    image_size: Tuple[int, int] = (1280, 720)  # 720p
    fps: int = 30  # 强制30fps
...
def force_720P_30fps(device_path):
    """强制设置720P 30fps模式"""
    print("🚀 强制启用720P@30fps模式...")
...
class HighFPSIMX415:
    def __init__(self, config: IMX415CameraConfig = None):
        ...
        self.config = config
        self.capture = None
        self.device_count = len(config.device_paths) if config.device_paths else 1
        
        if config.device_paths is None:
            self.config.device_paths = ['/dev/video0']
        
        print(f"🎯 目标: 720P@{config.fps}fps")
        self._setup_high_fps_camera()
        
        if config.real_time_view:
            cv2.namedWindow("720P@30fps", cv2.WINDOW_NORMAL)
            cv2.resizeWindow("720P@30fps", 960, 540)
    ...
    def _setup_high_fps_camera(self):
        ...
        # 第一步: 强制V4L2设置
        if not force_720P_30fps(device_path):
...
def get_imx415_frame(imx415_module: HighFPSIMX415) -> List[Tuple[np.ndarray, np.ndarray]]:
    """Get RGB data from all cameras"""
    frame = imx415_module.get_frame()
    return [frame]
