...
class TrajectoryRecorder:
    ...
    def save_trajectory(self, task, key_input):
        ...
        assert len(key_input) == 2, f"len of key_input over 2"
...
def main(task, path, frequency, rgb_width=640, rgb_height=480, fps=30, gui=False, device_paths=None, enable_realsense=True, enable_imx415=True, debug_sensor=False, home_target=None):
    ...
    try:
        ...
        K = np.multiply(robot.info().K_x_nom, 0.02)
        ...
        # robot.SetMaxContactWrench([30,30,30,30,30,30])
        ...
        while True:
            ...
            # cmd_gripper_pos = np.clip(cmd_gripper_pos, 0, 0.14)
            cmd_leader_gripper_pos = (1-cmd_gripper_pos)*0.14
            cmd_leader_gripper_pos *=3
            cmd_leader_gripper_pos = np.clip(cmd_leader_gripper_pos, 0, 0.14)
            # cmd_leader_gripper_pos = np.clip(cmd_leader_gripper_pos, 0, 0.14)
            ...
            if not debug_sensor:
                ...
                robot.SendCartesianMotionForce([*cmd_end_effector_pos, cmd_quat_numpy.w, cmd_quat_numpy.x, cmd_quat_numpy.y, cmd_quat_numpy.z])
                gripper.Move(cmd_leader_gripper_pos, 0.15, 100)
...
if __name__ == "__main__":
    ...
    #default_path = f"/media/forceyqj/My PSSD/601_flexiv_record_data/assemble_gears/xiajunjie_little/{current_date}/"
    #default_path =f"/media/forceyqj/My Book/601_flexiv_record_data/push_bottle_with_diff_height/{current_date}/"
    default_path =f"/media/forceyqj/My PSSD/601_flexiv_record_data/cleanse_the_surface/liyang/{current_date}/"
