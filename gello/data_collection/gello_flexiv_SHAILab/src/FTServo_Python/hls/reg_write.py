...
packetHandler = hls(portHan = packetHandler.RegWritePosEx(scs_id, 4095, 60, 50, 500)
...
while 1:
    ...
    for scs_id in range(1, 11):
        scs_comm_result, scs_errorandler.getTxRxResult(scs_comm_result))
        if scs_error != 0:
    ...
    # torque=500
    for scs_id in range(1, 11):
        scs_comm_result, scs_error = packetHandler.RegWritePosEx(scs_id, 0, 60, 50, 500)
        if scs_comm_result != COMM_SUCCESS:
