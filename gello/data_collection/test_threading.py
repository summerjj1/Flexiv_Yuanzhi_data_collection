...
def camera_thread(camera_id, serial_num, log_queue):
    """
    Thread to capture frames from a Realsense camera and log timestamps.
    Args:
        camera_id (int): Identifier for the camera (e.g., 1 or 2).
        serial_num (str): Serial number of the Realsense camera.
        log_queue (queue.Queue): Queue to store timestamp logs.
    """
    try:
...
def main():
    ...
    threads = [
        threading.Thread(target=camera_thread, args=(1, serials[0], log_queue)),
        threading.Thread(target=camera_thread, args=(2, serials[1], log_queue))
    ]
    ...
    # Analyze logs to verify concurrency
