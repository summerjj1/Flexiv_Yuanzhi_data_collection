...
# from REAL2SIM_GELLO_AIR_v1 import init_hls_port, read_8_positions_rad
# from REAL2SIM_GELLO_AIR_v1 import Robotic as LeaderArmGello
from gello_flexiv_SHAILab.src.REAL2SIM_GELLO_AIR_v1 import init_hls_port, read_8_positions_rad
from gello_flexiv_SHAILab.src.REAL2SIM_GELLO_AIR_v1 import Robotic as LeaderArmGello
...
import inspect
print("read_8_positions_rad from module:", read_8_positions_rad.__module__)
try:
    print("function file:", inspect.getsourcefile(read_8_positions_rad))
...
def on_press(key):
    ...
    try:
        ...
        if record_keyinput_flag and key.char != 'c':
            print(f'按下了: {key.char}')
            key_input.append(time.time())
    except AttributeError:
        if record_keyinput_flag and key != keyboard.Key.ctrl:
            print(f'按下了特殊键: {key}')
            key_input.append(time.time())
...
class TrajectoryRecorder:
    def __init__(self, output_file, realsense_config=None, robot_config=None):
        ...
        # RealSense camera data - COMMENTED OUT
        ...
        # Initialize RealSense cameras - COMMENTED OUT
    ...
    #     if len(self.serial_numbers) != 3:
    #         self.logger.warn("Expected 3 RealSense cameras, found fewer. Some features may not work.")
    #         assert len(self.serial_numbers) == 3, "Expected exactly 3 RealSense cameras, found fewer. Some features may not work."
    #         return []
    ...
    def _tac_loop(self):
        ...
        fps = 30  # Default fps since camera is commented out
        interval = 1.0 / fps
    ...
    def align_frames(self):
        ...
        # Validate RealSense camera data consistency - COMMENTED OUT
        ...
        self.logger.info(f"Data validation passed: "
                        ...
                        # f"RealSense frames: {total_realsense_frames}")  # COMMENTED OUT
        
        # Log RealSense statistics - COMMENTED OUT
    ...
    def save_trajectory(self, task, key_input):
        ...
        # Stop camera thread - COMMENTED OUT
        # self._camera_running = False
        # if self._camera_thread and self._camera_thread.is_alive():
        #     self._camera_thread.join(timeout=2.0)
        # self.logger.info("Camera thread stopped")
        
        # Then cleanup camera pipelines (must be after thread stops) - COMMENTED OUT
        ...
        with h5py.File(self.output_file, 'w', libver='latest', rdcc_nbytes=1024*1024*100) as hf:
            ...
            # RealSense camera timestamps (~30Hz from camera thread) - COMMENTED OUT
            # if self.num_realsense_cameras > 0:
            ...
            # hf.attrs['num_realsense_cameras'] = self.num_realsense_cameras  # COMMENTED OUT
            # hf.attrs['num_realsense_frames'] = len(self.camera_timestamps) if self.num_realsense_cameras > 0 else 0  # COMMENTED OUT
            hf.attrs['creation_date'] = time.strftime("%Y-%m-%d %H:%M:%S")
            # Legacy compatibility
            # hf.attrs['num_cameras'] = self.num_realsense_cameras  # COMMENTED OUT
            # hf.attrs['num_camera_frames'] = len(self.camera_timestamps) if self.num_realsense_cameras > 0 else 0  # COMMENTED OUT
            
            # Save per-camera statistics for RealSense - COMMENTED OUT
            # for cam_name in self.realsense_roles:
            ...
            # Save images asynchronously - COMMENTED OUT
            # threads = []
            
            # Save RealSense images - COMMENTED OUT
            # for cam_name in self.realsense_roles:
            ...
            # Wait for all image saving to complete - COMMENTED OUT
            # for i in threads:
            #     thread.join()
        
        # Calculate overall camera statistics - COMMENTED OUT
        # total_realsense_frames = len(self.camera_timestamps) if self.num_realsense_cameras > 0 else 0
        # realsense_valid_counts = {}
        ...
        # Save MP4 videos for each camera - COMMENTED OUT
...
def main(task, path, frequency, rgb_width=640, rgb_height=480, fps=30, gui=False, enable_realsense=True, debug_sensor=False, home_target=None):
    ...
    # Initialize camera configurations - COMMENTED OUT
    ...
    # init_qjoint = [0.08889224380254745, -0.7063623666763306, 0.06017714738845825, 1.2980624437332153, -0.10702542960643768, 0.5192122459411621, 0.1847415268421173]
    ...
    # recorder.start_camera_thread()  # COMMENTED OUT
    ...
    while True:
        ...
        print("pos8pos8pos8pos8", pos8)
        # import sys
        # sys.exit(-1)
        print("origin target_positions: ", pos8[0])
        # target_positions = pos8[:-1].tolist() # zhqw modify 251219
        ...
        print("send target_positions: ", target_positions)
        ...
        distance = abs(np.sum(abs(current_tcp_pos - np.array(home_target[:3]))))
    ...
    # recorder.set_camera_recording(True)  # COMMENTED OUT
    ...
    GRIPPER_HOLD_STEPS = 3     # number of loop iterations to keep the same command
    ...
    while True:
        ...
        if not record_keyinput_flag:
            right_robot.joint_position_control_with_impedance(target_positions, 0.1, vel_ratio=0.3, acc_ratio=0.3)
        else:

            print('Finishing recording ')
            right_robot.robot.SwitchMode(mode.NRT_CARTESIAN_MOTION_FORCE)
            right_robot.robot.SendCartesianMotionForce(
                    [float(home_target[0]), float(home_target[1]), float(home_target[2] + 0.1),
            ...
            right_robot.gripper.Move(0.1, 0.1, 50)
            # recorder._camera_running = False  # COMMENTED OUT
        ...
        # recorder.add_action(current_tcp_pos, current_tcp_quat, gripper_width)
...
if __name__ == "__main__":
    ...
    # Handle camera enable/disable flags - COMMENTED OUT
    # enable_realsense = args.enable_realsense or not args.disable_realsense
    enable_realsense = False  # Disable camera by default
