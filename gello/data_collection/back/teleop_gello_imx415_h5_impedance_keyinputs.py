...
def on_press(key):
    try:
        if  key.char is not 'c':
            print(f'按下了: {key.char}')
            key_input.append(time.time())
    except AttributeError:
        if key is not keyboard.Key.ctrl:
            print(f'按下了特殊键: {key}')
            key_input.append(time.time())
...
def main(task, path, frequency, rgb_width=640, rgb_height=480, fps=30, gui=False, device_paths=None, enable_realsense=True, enable_imx415=True, debug_sensor=False):
    ...
    recorder = TrajectoryRecorder(output_file, imx415_config, realsense_config, robot_config=robot_config)
    ...
    try:
        ...
        K_5 = np.multiply(robot.info().K_x_nom, 0.2)
        robot.SetCartesianImpedance(K_5)
...
if __name__ == "__main__":
    ...
    default_path = f"/media/forceyqj/My Book/601_flexiv_record_data/clean_the_board/{current_date}/"
