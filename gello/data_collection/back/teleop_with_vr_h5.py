...
def main(task, path, frequency, rgb_width=640, rgb_height=480, fps=30):
    ...
    try:
        ...
        while True:
            # Get latest states from high-frequency thread
            robot_states, gripper_states, state_timestamp = recorder.get_latest_states()
            
            if robot_states is None or gripper_states is None:
                logger.warn("Waiting for state thread to initialize...")
                time.sleep(0.01)
                continue
            ...
            # State recording is now handled automatically by state thread at ~1000Hz
            # Camera data is automatically recorded by background thread at fixed fps
            ...
            if current_input.get('rightHand', 0) > 0.5:
                if not is_initialized:
                    ...
                    is_initialized = True
                    # Enable state recording in background thread
                    recorder.set_state_recording(True)
                    logger.info("Start to teleoperate and record data...")
