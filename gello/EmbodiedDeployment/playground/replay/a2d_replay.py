...
def load_hdf5(dataset_path):
    ...
    try:
        with h5py.File(dataset_path, 'r') as root:
            qposes = root.get('/state/joint/position')
            eefs = root.get('/state/effector/position')
            return qposes , eefs
...
def example_a2d_robot(lmdb_path):
    ...
    new_data = move_data
    robot.move(new_data)
    qpose, eefs = load_hdf5(lmdb_path)
    rate = Rate(30) #robot.controllers["a2d_dual_arm"].args.frame_rate)
...
if __name__ == "__main__":
    example_a2d_robot( 
        "/workspace/dataset/raw_joints.h5"
    )