...
def matmul_small_bias_res(
    ...
    for p in range(pid, grid_i * grid_j, psize):
        ...
        acc = tl.load(
            res_ptr
            + (i + tl.arange(0, BLOCK_SIZE_N))[:, None] * hidden
...
def matmul_small_bias_gelu(
    ...
    for p in range(pid, grid_i * grid_j, psize):
        ...
        for k in range(0, features, BLOCK_SIZE_K):
            ...
            acc = tl.dot(x, w, acc)
        acc = acc * tl.sigmoid(
            1.5957691216057308 * acc * (1 + 0.044715 * acc * acc)
        )
        tl.store(
            out_ptr
            + (i + tl.arange(0, BLOCK_SIZE_N))[:, None] * hidden
...
def layer_norm_small_kernel(
    ...
    for i in range(pid, seq_len, psize):
        ...
        x_centered = x - mean
        var = tl.sum(
            x_centered * x_centered * (tl.arange(0, MAX_LEN) < features)
        ) * (1.0 / features)
...
def layer_norm_n256_1152(x, norm_w, norm_b, out):
    ...
    layer_norm_small_kernel[
        seq_len,
    ](x, out, norm_w, norm_b, seq_len=seq_len, features=1152)


def layer_norm_QKV_matmul_n256_1152_3456_bias(
...
@triton.jit
def matmul_512x1152x1152_twopart_bias_res(
    old_ptr,
    inp_ptr,
    weight_ptr,
    bias_ptr,
    ...
    for p in range(pid, grid_i * grid_j, psize):
        ...
        acc += tl.load(
            bias_ptr + (j + tl.arange(0, BLOCK_SIZE_M1))[None, :]
        ).to(tl.float32)
    ...
    for p in range(pid, grid_i * grid_j * grid_k, psize):
        ...
        if k0 == 0:
            acc += tl.load(
                old_ptr
                + (i + tl.arange(0, BLOCK_SIZE_N2))[:, None] * hidden
                + (j + tl.arange(0, BLOCK_SIZE_M2))[None, :]
            ).to(tl.float32)
            acc += tl.load(
                bias_ptr + (j + tl.arange(0, BLOCK_SIZE_M2))[None, :]
            ).to(tl.float32)
        ...
        tl.store(
            ...
            * ((hidden - hidden1) * grid_k)
            + (j - hidden1 + tl.arange(0, BLOCK_SIZE_M2))[None, :]
            + k0 * (hidden - hidden1),
            acc.to(tl.bfloat16),
            mask=((i + tl.arange(0, BLOCK_SIZE_N2))[:, None] < seq_len)
            & ((j + tl.arange(0, BLOCK_SIZE_M2))[None, :] < hidden),
...
@triton.jit
def combine_1536_1152_twopart(
    out_ptr, inp_ptr, seq_len: tl.constexpr, hidden: tl.constexpr
):
    pid = tl.program_id(0)
    psize = tl.num_programs(0)
...
def layer_norm_matmul_n256_1152_4304_bias_gelu(
...
@triton.jit
def rms_norm_kernel(
    inp_ptr, out_ptr, seq_len: tl.constexpr, features: tl.constexpr
):
...
def rms_matmul_n_2048_16384_gate(x, weight1, weight2, out, x_norm):
    ...
    matmul_small_gate[((seq_len + 127) // 128, (16384 + 63) // 64)](
        x_norm, weight1, weight2, out, seq_len, 2048, 16384
    )
...
def matmul_n_16384_2048_res(x, weight, out):
    ...
    if seq_len < 512:
        BLOCK_SIZE_N = 64
    matmul_small_res[
        ((seq_len + BLOCK_SIZE_N - 1) // BLOCK_SIZE_N) * (2048 // 64),
    ](
        x,
        weight,
        out,
...
def layer_norm_matmul_n256_1152_2048_bias(
...
def rms_matmul_n_2048_2560_qkv_rope(
    x, weight_qkv, rope_weight, Q, K, V, x_norm
):
    seq_len = x.shape[0]
    rms_norm_kernel[(seq_len,)](x, x_norm, seq_len, 2048)
    ...
    for i in range(18):
        ...
        if i != 17:
            ...
            matmul_n_2048_2048_res(
                attn, weights["encoder_attn_o_w"][i], buffers["encoder_x"]
            )
...
def matmul_1_32_1024_bias(x, weight, bias, out):
    matvec_bias_kernel[((1024 + 7) // 8,)](
        x, weight, bias, out, features=32, hidden=1024
    )
...
def scaled_matmul_rope_qkv(
    ...
    while pid < grid_m * grid_n:
        ...
        for k in range(0, features, BLOCK_SIZE_K):
            ...
            w = tl.load(
                weight_qkv_ptr
                + offs_k[:, None] * ((num_heads + 2) * head_dim)
                + offs_j,
                mask=offs_k[:, None] < features,
                other=0,
            )
...
def rms_matmul_k_1024_2560_qkv_rope(
    x, weight_qkv, rope_weight, Q, K, V, x_norm_factor
):
    seq_len = x.shape[0]
...
def softmax_kernel_mask0(
    ...
    assert (
        BLOCK_SIZE >= queries
    ), f"BLOCK_SIZE must be >= N, got {BLOCK_SIZE} < {queries}"
    ...
    for i in range(pid * BLOCK_SIZE_M, queries, psize * BLOCK_SIZE_M):
        ...
        attn_mask = attn_mask & (
            (offs_i >= num_heads) | (offs_j <= encoder_seq_len)
        )
        vals = tl.load(
...
def matmul_k8_256_n_softmax_mask0(Q, K, out, encoder_seq_len):
    ...
    head_dim = 256
    matmul_abT_scale[
        (((total_queries + 31) // 32) * ((total_keys + 31) // 32),)
    ](
        Q,
        K,
        out,
    ...
    softmax_kernel_mask0[((total_queries + 3) // 4,)](
        out,
        total_queries,
        total_keys,
        8,
...
class Pi0Inference:
    def __init__(self, checkpoint, num_views, chunk_size):
        ...
        self.weights = {
            "vision_patch_embedding_w": torch.empty(
                14, 14, 3, 1152, dtype=torch.bfloat16, device="cuda"
            ),
            "vision_patch_embedding_b": torch.empty(
                1152, dtype=torch.bfloat16, device="cuda"
            ),
            "vision_position_embedding": torch.empty(
                256, 1152, dtype=torch.bfloat16, device="cuda"
            ),
            "vision_attn_qkv_w": torch.empty(
                27, 1152, 3 * 1152, dtype=torch.bfloat16, device="cuda"
            ),
            "vision_attn_qkv_b": torch.empty(
                27, 3 * 1152, dtype=torch.bfloat16, device="cuda"
            ),
            "vision_attn_o_w": torch.empty(
                27, 1152, 1152, dtype=torch.bfloat16, device="cuda"
            ),
            "vision_attn_o_b": torch.empty(
                27, 1152, dtype=torch.bfloat16, device="cuda"
            ),
            "vision_ffn_up_w": torch.empty(
                27, 1152, 4304, dtype=torch.bfloat16, device="cuda"
            ),
            "vision_ffn_up_b": torch.empty(
                27, 4304, dtype=torch.bfloat16, device="cuda"
            ),
            "vision_ffn_down_w": torch.empty(
                27, 4304, 1152, dtype=torch.bfloat16, device="cuda"
            ),
            ...
            "vision_final_norm_w": torch.empty(
                1152, dtype=torch.bfloat16, device="cuda"
            ),
            "vision_final_norm_b": torch.empty(
                1152, dtype=torch.bfloat16, device="cuda"
            ),
            "encoder_multi_modal_projector_w": torch.empty(
                1152, 2048, dtype=torch.bfloat16, device="cuda"
            ),
            "encoder_multi_modal_projector_b": torch.empty(
                2048, dtype=torch.bfloat16, device="cuda"
            ),
            "encoder_attn_qkv_w": torch.empty(
                18, 2048, 2560, dtype=torch.bfloat16, device="cuda"
            ),
            "encoder_attn_o_w": torch.empty(
                18, 2048, 2048, dtype=torch.bfloat16, device="cuda"
            ),
            "encoder_ffn_gate_w": torch.empty(
                18, 2048, 16384, dtype=torch.bfloat16, device="cuda"
            ),
            "encoder_ffn_up_w": torch.empty(
                18, 2048, 16384, dtype=torch.bfloat16, device="cuda"
            ),
            "encoder_ffn_down_w": torch.empty(
                18, 16384, 2048, dtype=torch.bfloat16, device="cuda"
            ),
            "decoder_state_in_proj_w": torch.empty(
                32, 1024, dtype=torch.bfloat16, device="cuda"
            ),
            "decoder_state_in_proj_b": torch.empty(
                1024, dtype=torch.bfloat16, device="cuda"
            ),
            "decoder_action_fused_in_proj_w": torch.empty(
                32, 1024, dtype=torch.bfloat16, device="cuda"
            ),
            "decoder_action_fused_time_biases": torch.empty(
                10, 1024, dtype=torch.bfloat16, device="cuda"
            ),
            "decoder_action_mlp_w": torch.empty(
                1024, 1024, dtype=torch.bfloat16, device="cuda"
            ),
            "decoder_action_mlp_b": torch.empty(
                1024, dtype=torch.bfloat16, device="cuda"
            ),
            "decoder_attn_qkv_w": torch.empty(
                18, 1024, 2560, dtype=torch.bfloat16, device="cuda"
            ),
            "decoder_attn_o_w": torch.empty(
                18, 2048, 1024, dtype=torch.bfloat16, device="cuda"
            ),
            "decoder_ffn_gate_w": torch.empty(
                18, 1024, 4096, dtype=torch.bfloat16, device="cuda"
            ),
            "decoder_ffn_up_w": torch.empty(
                18, 1024, 4096, dtype=torch.bfloat16, device="cuda"
            ),
            "decoder_ffn_down_w": torch.empty(
                18, 4096, 1024, dtype=torch.bfloat16, device="cuda"
            ),
            "decoder_action_fused_out_proj_w": torch.empty(
                1024, 32, dtype=torch.bfloat16, device="cuda"
            ),
            "decoder_action_fused_out_proj_b": torch.empty(
                32, dtype=torch.bfloat16, device="cuda"
            ),
            "language_embeds": torch.empty(
                self.prompt_len, 2048, dtype=torch.bfloat16, device="cuda"
            ),
        }
        ...
        self.buffers = {
            ...
            "observation_state_normalized": torch.empty(
                32, dtype=torch.bfloat16, device="cuda"
            ),
            "diffusion_noise": torch.empty(
                chunk_size, 32, dtype=torch.bfloat16, device="cuda"
            ),
            "vision_x": torch.empty(
                num_views, 256, 1152, dtype=torch.bfloat16, device="cuda"
            ),
            "vision_x_norm": torch.empty(
                num_views, 256, 1152, dtype=torch.bfloat16, device="cuda"
            ),
            "vision_QKV": torch.empty(
                num_views, 256, 3 * 1152, dtype=torch.bfloat16, device="cuda"
            ),
            "vision_hidden": torch.empty(
            ...
            "encoder_rope_weights": torch.empty(
                encoder_seq_len, 256, dtype=torch.bfloat16, device="cuda"
            ),
            "encoder_x": torch.empty(
                encoder_seq_len, 2048, dtype=torch.bfloat16, device="cuda"
            ),
            "encoder_x_norm": torch.empty(
                encoder_seq_len, 2048, dtype=torch.bfloat16, device="cuda"
            ),
            ...
            "encoder_Q": torch.empty(
                encoder_seq_len * 8, 256, dtype=torch.bfloat16, device="cuda"
            ),
            "encoder_hidden": torch.empty(
                encoder_seq_len, 16384, dtype=torch.bfloat16, device="cuda"
            ),
            "decoder_rope_weights": torch.empty(
                decoder_seq_len, 256, dtype=torch.bfloat16, device="cuda"
            ),
            "decoder_x": torch.empty(
                (decoder_seq_len, 1024), dtype=torch.bfloat16, device="cuda"
            ),
            "decoder_x_buf": torch.empty(
                (decoder_seq_len, 1024), dtype=torch.bfloat16, device="cuda"
            ),
            "decoder_state_buf": torch.empty(
                (1, 1024), dtype=torch.bfloat16, device="cuda"
            ),
            "decoder_norm_factor_buf": torch.empty(
                (decoder_seq_len,), dtype=torch.bfloat16, device="cuda"
            ),
            "decoder_q_buf": torch.empty(
                (decoder_seq_len * 8, 256), dtype=torch.bfloat16, device="cuda"
            ),
            "decoder_attn_buf": torch.empty(
                (decoder_seq_len * 8, encoder_seq_len + decoder_seq_len),
                dtype=torch.bfloat16,
                device="cuda",
            ),
            "decoder_hidden": torch.empty(
                (decoder_seq_len, 4096), dtype=torch.bfloat16, device="cuda"
            ),
            "decode_split_k_buf": torch.empty(
                (2, decoder_seq_len, 1024), dtype=torch.float32, device="cuda"
            ),
        }
        ...
        inv_freq = 1.0 / (
            10000
            ** (
                torch.arange(0, 256, 2, dtype=torch.float32, device="cuda")
                / 256
            )
        )
        ...
        position_ids = (
            torch.arange(decoder_seq_len, device="cuda") + encoder_seq_len
        )
        inv_freq = 1.0 / (
            10000
            ** (
    ...
    def record_run(self):
        self.buffers["encoder_x"][self.num_views * 256 :].copy_(
            self.weights["language_embeds"]
        )
    ...
    def forward(
        ...
        self.buffers["observation_state_normalized"].copy_(
            observation_state_normalized
        )
