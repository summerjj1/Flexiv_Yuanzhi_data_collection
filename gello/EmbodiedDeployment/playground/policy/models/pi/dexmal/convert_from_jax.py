...
def convert_weights(weights, dump_weights):
    ...
    vision_attn_qkv_w = torch.cat(
        [
            torch.tensor(
                dump_weights["PaliGemma"]["img"]["Transformer"][
                    "encoderblock"
            ...
            torch.tensor(
                dump_weights["PaliGemma"]["img"]["Transformer"][
                    "encoderblock"
                ]["MultiHeadDotProductAttention_0"]["key"]["kernel"]["value"],
                dtype=torch.bfloat16,
                device="cpu",
            ).flatten(2),
            torch.tensor(
    ...
    vision_attn_qkv_b = torch.cat(
        [
            torch.tensor(
                dump_weights["PaliGemma"]["img"]["Transformer"][
                    "encoderblock"
                ]["MultiHeadDotProductAttention_0"]["query"]["bias"]["value"],
                dtype=torch.bfloat16,
                device="cpu",
            ).flatten(1),
            torch.tensor(
                dump_weights["PaliGemma"]["img"]["Transformer"][
                    "encoderblock"
                ]["MultiHeadDotProductAttention_0"]["key"]["bias"]["value"],
                dtype=torch.bfloat16,
                device="cpu",
            ).flatten(1),
            torch.tensor(
                dump_weights["PaliGemma"]["img"]["Transformer"][
                    "encoderblock"
                ]["MultiHeadDotProductAttention_0"]["value"]["bias"]["value"],
                dtype=torch.bfloat16,
                device="cpu",
            ).flatten(1),
        ],
    ...
    weights["vision_ffn_down_w"].copy_(
        torch.tensor(
            dump_weights["PaliGemma"]["img"]["Transformer"]["encoderblock"][
                "MlpBlock_0"
    ...
    weights["vision_ffn_down_b"].copy_(
        torch.tensor(
            dump_weights["PaliGemma"]["img"]["Transformer"]["encoderblock"][
                "MlpBlock_0"
            ]["Dense_1"]["bias"]["value"],
    ...
    weights["vision_pre_attn_norm_w"].copy_(
        torch.tensor(
            dump_weights["PaliGemma"]["img"]["Transformer"]["encoderblock"][
                "LayerNorm_0"
            ]["scale"]["value"],
    ...
    weights["vision_pre_attn_norm_b"].copy_(
        torch.tensor(
            dump_weights["PaliGemma"]["img"]["Transformer"]["encoderblock"][
                "LayerNorm_0"
            ]["bias"]["value"],
    ...
    weights["vision_pre_ffn_norm_w"].copy_(
        torch.tensor(
            dump_weights["PaliGemma"]["img"]["Transformer"]["encoderblock"][
                "LayerNorm_1"
            ]["scale"]["value"],
    ...
    weights["vision_pre_ffn_norm_b"].copy_(
        torch.tensor(
            dump_weights["PaliGemma"]["img"]["Transformer"]["encoderblock"][
                "LayerNorm_1"
            ]["bias"]["value"],
    ...
    weights["vision_final_norm_w"].copy_(
        torch.tensor(
            dump_weights["PaliGemma"]["img"]["Transformer"]["encoder_norm"][
                "scale"
            ]["value"],
            dtype=torch.bfloat16,
            device="cpu",
    ...
    w_v *= 1 + w_scale[:, :, None]
    w_q = (
        w_q.reshape((18, 2048, 8, 2, 128))
        .transpose((0, 1, 2, 4, 3))
        .reshape((18, 2048, 2048))
    )
    w_k = (
        w_k.reshape((18, 2048, 2, 128))
        .transpose((0, 1, 3, 2))
        .reshape((18, 2048, 256))
    )

    weights["encoder_attn_qkv_w"] = torch.tensor(  # 18, 2048, 2560
        np.concatenate([w_q, w_k, w_v], axis=2),
        dtype=torch.bfloat16,
        device="cpu",
    )
    w_attn_o = (
        dump_weights["PaliGemma"]["llm"]["layers"]["attn"]["attn_vec_einsum"][
            "w"
        ]["value"]
        .reshape((18, 8 * 256, 2048))
        .astype("float32")
    )
    weights["encoder_attn_o_w"] = torch.tensor(  # 18, 2048, 2048
    ...
    w_gate = dump_weights["PaliGemma"]["llm"]["layers"]["mlp"][
        "gating_einsum"
    ]["value"][:, 0].astype("float32")
    ...
    weights["encoder_ffn_up_w"] = torch.tensor(
        w_up, dtype=torch.bfloat16, device="cpu"
    )
    weights["encoder_ffn_down_w"] = torch.tensor(
        w_down, dtype=torch.bfloat16, device="cpu"
    )

    # decoder weights
    w_scale = dump_weights["PaliGemma"]["llm"]["layers"][
        "pre_attention_norm_1"
    ]["scale"]["value"].astype("float32")

    w_q = dump_weights["PaliGemma"]["llm"]["layers"]["attn"]["q_einsum_1"][
        "w"
    ]["value"].astype("float32")
    w_q = w_q.transpose((0, 2, 1, 3)).reshape((18, 1024, 8 * 256))
    w_k = dump_weights["PaliGemma"]["llm"]["layers"]["attn"]["kv_einsum_1"][
        "w"
    ]["value"][:, 0, 0].astype("float32")
    w_v = dump_weights["PaliGemma"]["llm"]["layers"]["attn"]["kv_einsum_1"][
        "w"
    ]["value"][:, 1, 0].astype("float32")
    ...
    w_q = (
        w_q.reshape((18, 1024, 8, 2, 128))
        .transpose((0, 1, 2, 4, 3))
        .reshape((18, 1024, 2048))
    )
    w_k = (
        w_k.reshape((18, 1024, 2, 128))
        .transpose((0, 1, 3, 2))
        .reshape((18, 1024, 256))
    )
    ...
    w_attn_o = (
        dump_weights["PaliGemma"]["llm"]["layers"]["attn"][
            "attn_vec_einsum_1"
        ]["w"]["value"]
    ...
    w_gate = dump_weights["PaliGemma"]["llm"]["layers"]["mlp_1"][
        "gating_einsum"
    ]["value"][:, 0].astype("float32")
    w_up = dump_weights["PaliGemma"]["llm"]["layers"]["mlp_1"][
        "gating_einsum"
    ]["value"][:, 1].astype("float32")
    ...
    action_bias_contrib = torch.matmul(
        mlp_in_weight_action.T, action_in_proj_b
    )
...
def prepare_prompt(prompt: str, embedding_weight, tokenizer_path):
    ...
    embedding_weight_torch = torch.tensor(
        embedding_weight, dtype=torch.bfloat16, device="cuda"
    )
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="Convert JAX weights to PyTorch"
    )
    ...
    embedding_weight = dump_weights["PaliGemma"]["llm"]["embedder"][
        "input_embedding"
    ]["value"]
