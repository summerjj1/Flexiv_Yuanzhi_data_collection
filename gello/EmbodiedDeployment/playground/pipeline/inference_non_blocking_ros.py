...
from playground.my_robot.lift2_ros_dual_arm import Lift2Robot
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Simplified non-blocking inference client."
    )
    parser.add_argument(
        "--config-name",
        required=True,
...
def main() -> None:
    args = parse_args()
    rospy.init_node("policy_inference_client", anonymous=True)
    ...
    t = time.time()
    rate = rospy.Rate(config.model.action_rate_hz)
    step_history: List[int] = []
    action_history: List[np.ndarray] = []
    step_idx = 0
