...
# Ensure vendored piper_msgs (no catkin) is importable (kept for potential status use)
CURRENT_DIR = os.path.dirname(os.path.abspath(__file__))
PIPER_MSGS_DIR = os.path.join(CURRENT_DIR, "piper_msgs")
if PIPER_MSGS_DIR not in sys.path:
    sys.path.insert(0, PIPER_MSGS_DIR)
...
class RosOperator:
    ...
    def puppet_arm_publish(self, left, right):
        left_msg = self._to_joint_state(left)
        right_msg = self._to_joint_state(right)
        self.puppet_arm_left_publisher.publish(left_msg)
        self.puppet_arm_right_publisher.publish(right_msg)

    def _to_joint_state(self, joints):
        """Map 7-dim joint array to JointState message (position only)."""
        msg = JointState()
        msg.header = Header()
    ...
    def robot_base_publish(self, vel):
        ...
        if (
            getattr(self.args, "use_robot_base", False)
            and self.robot_base_publisher
        ):
            self.robot_base_publisher.publish(vel_msg)
        else:
            rospy.logwarn_throttle(
                5.0,
                "Base publish requested but base cmd topic is missing; ignoring",
            )
        ...
        while flag and not rospy.is_shutdown():
            ...
            for i in range(len(left)):
                if left_diff[i] < self.args.arm_steps_length[i]:
                    ...
                    left_arm[i] += (
                        left_symbol[i] * self.args.arm_steps_length[i]
                    )
                    flag = True
            for i in range(len(right)):
                if right_diff[i] < self.args.arm_steps_length[i]:
                    ...
                    right_arm[i] += (
                        right_symbol[i] * self.args.arm_steps_length[i]
                    )
                    flag = True

            left_msg = self._to_joint_state(left_arm)
            right_msg = self._to_joint_state(right_arm)
            self.puppet_arm_left_publisher.publish(left_msg)
            self.puppet_arm_right_publisher.publish(right_msg)
            step += 1
    ...
    def puppet_arm_publish_linear(self, left, right):
        ...
        for i in range(len(traj_left_list)):
            ...
            left_msg = self._to_joint_state(traj_left)
            right_msg = self._to_joint_state(traj_right)
            self.puppet_arm_left_publisher.publish(left_msg)
            self.puppet_arm_right_publisher.publish(right_msg)
            rate.sleep()
    ...
    def get_frame(self):
        ...
        if (
            ...
            or self.img_right_deque[-1].header.stamp.to_sec() < frame_time
        ):
            return False
        if (
            len(self.img_front_deque) == 0
            or self.img_front_deque[-1].header.stamp.to_sec() < frame_time
        ):
            return False
        if (
        ...
        if self.args.use_depth_image and (
            ...
            return False
        if self.args.use_depth_image and (
            len(self.img_right_depth_deque) == 0
        ...
        if self.args.use_depth_image and (
            ...
            return False
        if (
            self.args.use_robot_base
            and self.robot_base_deque is not None
            and (
                len(self.robot_base_deque) == 0
        ...
        while (
            self.puppet_arm_right_deque[0].header.stamp.to_sec() < frame_time
        ):
        ...
        if self.args.use_depth_image:
            while (
                self.img_left_depth_deque[0].header.stamp.to_sec() < frame_time
            ):
        ...
        if self.args.use_depth_image:
            while (
                self.img_right_depth_deque[0].header.stamp.to_sec()
                < frame_time
            ):
        ...
        robot_base = None
        if self.args.use_robot_base and self.robot_base_deque is not None:
    ...
    def init_ros(self):
        ...
        if getattr(self.args, "robot_base_topic", ""):
            rospy.Subscriber(
            ...
            self.robot_base_deque = None
            rospy.logwarn(
                "robot_base_topic missing; base odom subscription disabled"
            )
        ...
        if getattr(self.args, "robot_base_cmd_topic", ""):
            ...
            self.robot_base_publisher = None
            rospy.logwarn(
                "robot_base_cmd_topic missing; base cmd publish disabled"
            )
