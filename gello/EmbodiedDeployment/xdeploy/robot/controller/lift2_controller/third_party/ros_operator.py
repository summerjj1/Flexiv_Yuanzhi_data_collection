...
class RosOperator:
    def __init__(self, args, config, in_collect=False):
        ...
        # Ensure ROS node is initialized when this operator is created.
        # Some call sites (e.g. playground examples) do not call init_node()
        # beforehand, so we lazily initialize here if needed.
        if not rospy.core.is_initialized():
            rospy.init_node("lift2_ros_operator", anonymous=True)
    ...
    def follow_arm_publish(self, left, right):
        ...
        self.follow_arm_left_publisher.publish(
            joint_state_msg
        )  # /joint_control
        if len(right) != 0:
            joint_state_msg.joint_pos = right
            self.follow_arm_right_publisher.publish(
                joint_state_msg
            )  # /joint_control2
    ...
    def robot_base_control_thread(self):  # inference init robot arm in qpos
        ...
        while (not rospy.is_shutdown()) and self.base_enable:
            ...
            # 更新控制命令
            control.chx = pid_controllers["x"].update(
                current_x, target_x, dt=0.017
            )
            control.chy = pid_controllers["y"].update(
                current_y, target_y, dt=0.017
            )
        ...
        if not self.base_enable:
            ...
            plot_path = (
                os.path.join(
                    self.args.ckpt_dir, f"{self.args.ckpt_name}_PID.png"
                )
    ...
    def get_observation(self, ts=-1):  # get the robot observation
        ...
        # 保存机械臂状态
        follow_arm_left_eef_array = [
            arm_data["follow_arm_left_eef"].x,
            arm_data["follow_arm_left_eef"].y,
            arm_data["follow_arm_left_eef"].z,
            arm_data["follow_arm_left_eef"].roll,
            arm_data["follow_arm_left_eef"].pitch,
            arm_data["follow_arm_left_eef"].yaw,
            arm_data["follow_arm_left_eef"].gripper,
        ]

        follow_arm_right_eef_array = [
            arm_data["follow_arm_right_eef"].x,
            arm_data["follow_arm_right_eef"].y,
            arm_data["follow_arm_right_eef"].z,
            arm_data["follow_arm_right_eef"].roll,
            arm_data["follow_arm_right_eef"].pitch,
        ...
        obs_dict["qpos"] = np.concatenate(
            (
                np.array(arm_data["follow_arm_left"].joint_pos),
                np.array(arm_data["follow_arm_right"].joint_pos),
            ),
            axis=0,
        )
        obs_dict["qvel"] = np.concatenate(
            (
                np.array(arm_data["follow_arm_left"].joint_vel),
                np.array(arm_data["follow_arm_right"].joint_vel),
            ),
            axis=0,
        )
        obs_dict["effort"] = np.concatenate(
            (
                np.array(arm_data["follow_arm_left"].joint_cur),
                np.array(arm_data["follow_arm_right"].joint_cur),
            ),
            axis=0,
        )
        obs_dict["eef"] = np.concatenate(
            (follow_arm_left_eef_array, follow_arm_right_eef_array), axis=0
        )
    ...
    def get_action(self):
        ...
        action_dict["action_eef"] = np.concatenate(
            (master_arm_left_eef_array, master_arm_right_eef_array), axis=0
        )
